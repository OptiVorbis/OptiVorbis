...
The minimum supported Rust version (MSRV) for every package in this repository is 1.94. Bumping this version is not considered a breaking change for semantic versioning purposes. We will try to do it only when we estimate that such a bump would not cause widespread inconvenience or breakage.
